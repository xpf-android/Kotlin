...
public class HiTabTopLayout extends HorizontalScrollView implements IHiTabLayout<HiTabTop, HiTabTopInfo<?>> {
    ...
    public HiTabTop findTab(@NonNull HiTabTopInfo<?> info) {
        ViewGroup ll = getRootView(false);
    ...
    public void inflateInfo(@NonNull List<HiTabTopInfo<?>> infoList) {
        ...
        LinearLayout linearLayout = getRootView(true);
        selectedInfo = null;
    ...
    /**
     * 自动滚动，实现点击的位置能够自动滚动以展示前后2个tab
     *
     * @param nextInfo 点击tab对应的info
     */
    private void autoScroll(HiTabTopInfo<?> nextInfo) {
        ...
        //判断点击了屏幕左侧还是右侧
        if ((loc[0] + tabWidth / 2) > HiDisplayUtil.getDisplayWidthInPx(getContext()) / 2) {//点击右侧，向左滚动
    ...
    private int scrollWidth(int index, boolean toRight) {
        ...
        if (toRight) {//点击屏幕右侧
            ...
            if (rect.left <= -tabWidth) {//left坐标小于等于负控件的宽度，说明完全没有显示
                return tabWidth;
    ...
    private LinearLayout getRootView(boolean clear) {
        ...
        if (rootView == null) {
            ...
            rootView.removeAllViews();//清除子view
        }
        return rootView;
    }
}
